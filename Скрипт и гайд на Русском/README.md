...
- **Уведомления** — webhook-уведомления об истекающих/истёкших сертификатах
...
## Как скрипт решает проблему отзыва сертификатов

**Проблема:** Traefik не имеет встроенного механизма отзыва клиентских сертификатов. После того как сертификат добавлен в доверенные через `caFiles`, нет штатного способа заблокировать одного клиента без удаления всего CA или перезагрузки с новой конфигурацией.

**Решение:** Скрипт создаёт **отдельный промежуточный CA для каждого клиентского сертификата**. Вместо того чтобы доверять отдельным клиентским сертификатам напрямую, Traefik доверяет **bundle-файлу** (`clients-bundle.crt`), который содержит только промежуточные CA *активных* (не отозванных) клиентов.

**Как работает отзыв на практике:**

1. При выпуске сертификата создаётся уникальный промежуточный CA, подписанный корневым CA. Клиентский сертификат подписывается этим промежуточным CA. Промежуточный CA добавляется в bundle-файл.
2. При отзыве сертификата скрипт просто **удаляет соответствующий промежуточный CA из bundle-файла**. Клиентский сертификат больше не является доверенным для Traefik — соединение отклоняется немедленно.
3. Конфигурация Traefik регенерируется, bundle-файл обновляется. При включённом file provider в Traefik изменения вступают в силу **без перезагрузки** — Traefik отслеживает файл и перезагружает конфигурацию автоматически.

**Почему этот подход работает:**

- Не нужна инфраструктура CRL или OCSP — Traefik не нужно настраивать для проверки списков отзыва
- Гранулярный отзыв — отзыв одного клиента не затрагивает остальных
- Мгновенный эффект — отозванный клиент блокируется сразу после перестроения bundle-файла
- Без перезагрузки Traefik — file provider обеспечивает hot-reload

Это ключевой принцип дизайна скрипта: **промежуточные CA на каждый клиент + bundle-файл = отзыв без CRL/OCSP и без перезагрузки Traefik**.
...
| `curl` | Webhook-уведомления | Только для уведомлений |
...
Проверяет все неотозванные сертификаты и сообщает об истекающих (в пределах `EXPIRY_WARN_DAYS`) и истёкших. Если настроен URL webhook-уведомлений, отправляется предупреждение.
