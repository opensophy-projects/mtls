...
ИСПОЛЬЗОВАНИЕ:
  ...
  # либо: sudo -i, затем bash ./mtls.sh <команда> [опции]
...
КОМАНДЫ:
  ...
  cert issue --service S --name N [--days D] [--note ТЕКСТ] [--pass P]
      ...
      S и N допускают толь��о латиницу, цифры, '.', '_' и '-', без "__"
...
cli_config() {
    ...
    case "$subcmd" in
        ...
        set)
            local key="${1:-}" val="${2:-}"
            [ -z "$key" ] && { cli_err "Использов��ние: config set <ключ> <значение>"; exit 1; }
            case "$key" in
                TRAEFIK_DYNAMIC_PATH)  TRAEFIK_DYNAMIC_PATH="$val" ;;
                CA_PATH)               CA_PATH="$val" ;;
                CLIENTS_PATH)          CLIENTS_PATH="$val" ;;
